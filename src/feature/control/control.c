...
#include "feature/control/control_event.h"
                             ...
                             int reason_code)
{
  const char *status;
  char reasons[64] = "";
  if (!control_event_is_interesting(EVENT_CIRCUIT_STATUS))
    return 0;
  tor_assert(circ);
                                   ...
                                   int purpose, const struct timeval *tv)
{
  ...
  char event_tail[160] = "";
  if (!control_event_is_interesting(EVENT_CIRCUIT_STATUS_MINOR))
    return 0;
  tor_assert(circ);
                            ...
                            int reason_code)
{
  ...
  if (!control_event_is_interesting(EVENT_STREAM_STATUS))
    return 0;
                             ...
                             int reason)
{
  ...
  if (!control_event_is_interesting(EVENT_OR_CONN_STATUS))
    return 0;
...
control_event_stream_bandwidth(edge_connection_t *edge_conn)
{
  struct timeval now;
  char tbuf[ISO_TIME_USEC_LEN+1];
  if (control_event_is_interesting(EVENT_STREAM_BANDWIDTH_USED)) {
...
{
  if (!control_event_is_interesting(EVENT_CIRC_BANDWIDTH_USED))
    return 0;
...
control_event_conn_bandwidth(connection_t *conn)
{
  ...
  if (!get_options()->TestingEnableConnBwEvent ||
      !control_event_is_interesting(EVENT_CONN_BW))
    return 0;
...
control_event_circuit_cell_stats(void)
{
  ...
  if (!get_options()->TestingEnableCellStatsEvent ||
      !control_event_is_interesting(EVENT_CELL_STATS))
    return 0;
...
control_event_bandwidth_used(uint32_t n_read, uint32_t n_written)
{
  ...
  if (control_event_is_interesting(EVENT_BANDWIDTH_USED)) {
...
control_event_logmsg(int severity, uint32_t domain, const char *msg)
{
  ...
  if (domain == LD_BUG && control_event_is_interesting(EVENT_STATUS_GENERAL) &&
      severity <= LOG_NOTICE) {
  ...
  if (event >= 0 && control_event_is_interesting(event)) {
    char *b = NULL;
    const char *s;
    if (strchr(msg, '\n')) {
      char *cp;
...
control_event_descriptors_changed(smartlist_t *routers)
{
  ...
  if (!control_event_is_interesting(EVENT_NEW_DESC))
    return 0;
...
{
  if (!control_event_is_interesting(EVENT_ADDRMAP))
    return 0;
                                           ...
                                           const char *event_string)
{
  smartlist_t *strs;
  char *s, *esc = NULL;
  if (!control_event_is_interesting(event) || !smartlist_len(statuses))
    return 0;
...
#define CONTROL_EVENT_STATUS_BODY(event, sev)                   \
  ...
  do {                                                          \
    va_list ap;                                                 \
    if (!control_event_is_interesting(event))                           \
      return 0;                                                 \
                                                                \
    va_start(ap, format);                                       \
                    ...
                    const char *status)
{
  ...
  if (!control_event_is_interesting(EVENT_GUARD))
    return 0;
...
control_event_conf_changed(const smartlist_t *elements)
{
  ...
  char *result;
  smartlist_t *lines;
  if (!control_event_is_interesting(EVENT_CONF_CHANGED) ||
